...
def measure(training_type):
    ...
    if benchmark_path is not None:
        benchmark_command = [sys.executable, '-u', benchmark_path, str(train_proc.pid), training_type]
        benchmark_proc = run_process(benchmark_command, "Benchmark")

        monitor_thread = threading.Thread(target=monitor_subprocess, args=(train_proc, benchmark_proc))
